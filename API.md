...
```bash
curl -X POST https://signage.example.com/api/slides \
...
curl -X PUT https://signage.example.com/api/slides/42 \
  -H "X-API-KEY: your_api_key_here" \
...
curl -X POST https://signage.example.com/api/slides/99/variables \
  -H "X-API-KEY: your_api_key_here" \
...
curl -X POST https://signage.example.com/api/playlists/1/slides \
  -H "X-API-KEY: your_api_key_here" \
  -H "Content-Type: application/json" \
  -d '{
    "slide_id": 42,
...
```bash
curl -X GET https://signage.example.com/api/storage \
  -H "X-API-KEY: your_api_key_here"
```
...
```bash
curl -X POST https://signage.example.com/api/storage/cleanup \
  -H "X-API-KEY: your_api_key_here"
```
